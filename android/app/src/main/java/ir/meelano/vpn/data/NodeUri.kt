        ...
        return when (scheme) {
            "ss" -> {
                ...
                base.copy(
                    ...
                    // mirrors Parser.php::finalize(): SIP002 carries the transport in ?type=... and the
                    // plugin hints in the same query; dropping them dialed every ss over bare tcp even
                    // when the share link said type=ws&host=..., which never connects.
                    network = q["type"]?.takeIf { it.isNotBlank() && it != "tcp" } ?: "tcp",
