...
/**
 * The one gate every app-facing endpoint shares. feed() used to hold this check privately, which
 * is exactly why feedback() shipped without it: anyone could POST forged ok/fail reports, cool a
 * VIP node into a ban, and tilt the fleet's regime votes. When access.feedKey is set, every app
 * call carries the key; when it is empty the host is intentionally open and nothing changes.
 */
function requireFeedKey()
{
...
{
    requireFeedKey();
    // Lower-case once for the limiter bucket: 'VIP' and 'vip' are the same feed, and separate
    // buckets per spelling would let an abuser multiply the per-IP limit just by varying the case.
    if (!Util::rateLimit('feed:' . strtolower($action), (int) Util::cfg('limits.reqPerMinPerIp', 60))) {
        Util::respond(['error' => 'slow_down', 'servers' => [], 'retryAfter' => 60], ['status' => 429, 'maxAge' => 0]);
        return;
    }

    $payload = $action === 'vip' ? Builder::vip() : Builder::free();
    // 'cache' is internal bookkeeping (cache state for THIS response): read it for the gzip decision,
    // then strip it. That also makes the bytes identical whether we stream the prebuilt .json.gz from
    // disk (no 'cache' key inside) or encode the array ourselves — same body under the same ETag,
    // which is what Vary+ETag caching promises.
    $cacheState = isset($payload['cache']) && is_array($payload['cache']) ? $payload['cache'] : null;
    unset($payload['cache']);
    ...
    if ($cacheState !== null && isset($cacheState['state']) && $cacheState['state'] === 'fresh') {
