...
final class Parser
{
    ...
    private static function tlsOf(array $params)
    {
        ...
        if ($sec === 'none' || $sec === '') {
            if (!empty($params['tls'])) {
                // The classic vmess JSON spells this flag as the *word* "tls", which boolParam's
                // 1/true/yes/on set does not cover - so every vmess+tls node used to lose its TLS
                // marker here and get dialed with security=none. Accept the spelled forms first.
                $t = strtolower(trim((string) $params['tls']));
                if ($t === 'tls' || $t === 'https') {
                    return 'tls';
                }
                if ($t === 'reality') {
