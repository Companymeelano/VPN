...
final class Builder
{
    ...
    public static function doBuild($kind, $ttl)
    {
        ...
        // `$tuned` may carry bookkeeping keys that are not node tunes (today `_regime`); meta must
        // count only real per-node patches, or "tuned" answers 1 in a run where nothing was tuned.
        $tunedCount = 0;
        foreach ($tuned as $k => $_v) {
            if ($k === '_regime') {
                continue;
            }
            $tunedCount++;
    ...
    private static function gatePass(array $nodes, array $aliveIds, array &$meta)
    {
        ...
        // Dns::lookup only resolves while a run budget is set — and Probe::run sets its own, in a
        // later step. Warm the gate host HERE: on a cold dns cache the lookup below used to return
        // null, every SocksTask got skipped, and socks nodes never saw a gate test at all.
        Dns::setBudget(30);
    ...
    private static function maskNames(array $n, $brand, $slot)
    {
        ...
        if ($n['proto'] === 'vmess' && strpos($n['raw'], 'vmess://') === 0) {
            ...
            if (is_array($d)) {
                ...
                // json_encode() returns false on invalid UTF-8, and a free-list `ps` field arrives in
                // exactly that shape often enough; the old line then published "vmess://" (a dead URI)
                // instead of a node with a substituted remark. Substitute, escape, give up last.
                $enc = json_encode($d, JSON_UNESCAPED_UNICODE | JSON_INVALID_UTF8_SUBSTITUTE);
                if ($enc === false) {
                    $enc = json_encode($d, JSON_INVALID_UTF8_SUBSTITUTE | JSON_UNESCAPED_SLASHES);
                }
                if ($enc !== false) {
